...
import { ArrowType, Binary,
    ...
    List, makeVector, Struct, Utf8, LargeUtf8, Utf8View, BinaryView,
    util, Vector, vectorFromArray, makeData, FixedSizeList, Field
...
describe(`Utf8ViewVector`, () => {
    const values = ['foo', 'bar', 'baz', 'foo bar', 'bar'];
    const vector = vectorFromArray(values, new Utf8/*View*/);

    test(`has utf8View type`, () => {
        expect(vector.type).toBeInstanceOf(Utf8/*View*/);
    });
...
describe(`BinaryViewVector`, () => {
    ...
    const vector = vectorFromArray(values, new Binary /* View */);

    test(`has binaryView type`, () => {
        expect(vector.type).toBeInstanceOf(Binary /* View */);
    });
...
function basicVectorTests(vector, values, extras) {
