...
file_list = lambda source_dir, pattern: [
    fname for fname in os.listdir(source_dir) if fnmatch.fnmatch(fname, pattern)
]

is_data_change = lambda c: c.get("nd_type") == "DataChangeConfirmed"


def date_ranged_file_name_matches(
    file_list, start_date_tup, end_date_tup, fmt, max_delta=12
):
    matches = []
...
class Service(object):
    ...
    def on_cache_request(self, client_uuid, msg_dict):
        ...
        try:
            return [
                dict(
                    nd_type="CacheResponse",
                    uuid=client_uuid,
                    cache_key=msg_dict.get("cache_key"),
