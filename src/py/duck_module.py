...
class DuckService(object):
    ...
    def command(self, msg_dict):
        # parquet scans do not produce a result set
        # ergo diff handler...
        logr.info(f"duck_module.command: {msg_dict}")
        try:
            self.duck_conn.execute(msg_dict["sql"])
        except duckdb.IOException as ex:
            logr.error(f"DuckService.command: {ex}")
        response = [dict(nd_type="CommandResult", query_id=msg_dict["query_id"])]
