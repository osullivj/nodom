...
parquet_list_func = functools.partial(
    nd_utils.file_list, nd_consts.PQ_DIR, "*_pd.parquet"
)
...
class DepthService(nd_utils.Service):
    def on_client_data_change(self, uuid, client_change):
        ...
        if is_scan_change(client_change):
            ...
            ranged_matches = nd_utils.date_ranged_file_name_matches(
            ...
            new_urls_val = [
                f"https://localhost/api/parquet/{pqfile}" for pqfile in ranged_matches
            ]
            ...
            return [
                dict(
                    nd_type="DataChange",
                    cache_key="scan_sql",
                    old_value=old_sql_val,
                    new_value=new_sql_val,
                ),
                dict(
                    nd_type="DataChange",
                    cache_key="scan_urls",
                    old_value=old_urls_val,
                    new_value=new_urls_val,
                ),
            ]
