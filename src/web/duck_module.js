...
// Canonical Duck URL to get DuckDB-WASM from CDN. Not cool when there's
// an outage. So we use Simon Willison's excellent download-esm via
// sh/ndlesm.bat to pull specific JS into the bld subdir.
...
function batch_materializer(qid, batch) {
  ...
  let buffer_offset = Module.ccall(
    "get_chunk_cpp",
    "number",
    ["string", "number"],
    [qid, buffer_size],
  ...
  let dbl_heap64 = new Float64Array(
    Module.HEAPU64.buffer,
    buffer_offset,
    buffer_size / 2,
  );
  let bui_heap64 = new BigUint64Array(
    Module.HEAPU64.buffer,
    buffer_offset,
    buffer_size / 2,
  );
  ...
  console.log(
    "batch_materializer: buffer_offset=" +
      buffer_offset +
      ", row_count=" +
      row_count +
      "\n",
